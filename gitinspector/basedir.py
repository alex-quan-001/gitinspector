...
def get_basedir_git(path=None):
	...
	bare_command = subprocess.Popen(["git", "rev-parse", "--is-bare-repository"], bufsize=0,
	...
	if isbare:
		absolute_path = subprocess.Popen(["git", "rev-parse", "--git-dir"], bufsize=0, stdout=subprocess.PIPE).stdout
	else:
		absolute_path = subprocess.Popen(["git", "rev-parse", "--show-toplevel"], bufsize=0,
