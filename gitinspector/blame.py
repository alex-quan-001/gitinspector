...
class BlameThread(threading.Thread):
	...
	def run(self):
		git_blame_r = subprocess.Popen(self.blame_command, bufsize=0, stdout=subprocess.PIPE).stdout
...
class Blame(object):
	def __init__(self, repo, hard, useweeks, changes):
		self.blames = {}
		ls_tree_p = subprocess.Popen(["git", "ls-tree", "--name-only", "-r", interval.get_ref()], bufsize=0,
